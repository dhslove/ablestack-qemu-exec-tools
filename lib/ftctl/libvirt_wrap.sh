...
ftctl_command_requires_lock() {
  ...
  case "${command}" in
    reconcile)
      return 1
      ;;
    status|check|health|events|protect-start|preflight-remote|dr-key-ensure|dr-key-install|dr-key-remove|dr-status|dr-capabilities|dr-target-materialized|dr-cutover-commit)
      return 1
      ;;
