...
dispatch() {
  case "${CLI_COMMAND}" in
    ...
    dr-failback-commit)
      ...
      ;;
    dr-failback-abort)
      ftctl_dr_runtime_failback_abort "${CLI_PLAN}" "${CLI_RUN}" "${CLI_CUTOVER_SESSION_ID}" "${CLI_JSON}"
      ;;
    dr-status)
