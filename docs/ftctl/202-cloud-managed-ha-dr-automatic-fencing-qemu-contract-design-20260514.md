...
For DR failback, Cloud must also own the internal target Mold context and target primary VM lifecycle. The implemented source-controller UI uses the original-primary/source context and must not expose a target-Mold selector until Cloud can provision and transfer ownership to that selected target, as refined in [206. DR Cloud-Managed Failback Target Mold Design](206-dr-cloud-managed-failback-target-mold-design-20260516.md) and [211. DR Failback Action UX And Controller Model Design](211-dr-failback-action-ux-controller-model-design-20260521.md). If the source Mold is unavailable, the replica Mold must use the replica-site controller model from [208. DR Replica-Site Disaster Failback And Adoption Design](208-dr-replica-site-disaster-failback-and-adoption-design-20260517.md).
