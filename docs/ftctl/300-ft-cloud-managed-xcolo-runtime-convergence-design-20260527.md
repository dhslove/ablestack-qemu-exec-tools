...
Later validation with `i-2-54-VM` and `i-2-82-VM` proved that the then-current `primary.cont_before_migrate` experiment briefly made the primary report `running=true`, but the pair still timed out in `finish-migrate` / `inmigrate`. Secondary active overlays remained nearly empty while the primary overlay recorded writes. That follow-up is handled by [307. FT X-COLO Primary Quorum Remote Child Design](307-ft-xcolo-primary-quorum-remote-child-design-20260528.md): primary disk replacement must use a quorum that already contains both local primary active and remote NBD children. Design 332 later supersedes the explicit pre-migrate `cont` experiment and returns the start sequence to QEMU's documented `migrate-set-capabilities` then `migrate` flow.
...
Historical note: this design previously required QMP `cont` after all secondary NBD exports, primary block graph nodes, primary network filters, migration capabilities, and checkpoint delay were configured. That was an experiment to force the generated primary out of the paused state before `migrate`. Design 332 supersedes it: the generated primary stays paused and qemu FTCTL issues `migrate-set-capabilities` followed directly by `migrate`, matching the QEMU COLO startup sequence.
